...
public class ItemCreationsTests extends ApiConfig{
  ...
  @Test
  public void createDuplicateBody() throws InterruptedException {
    ...
    String statusFirst =
    given().contentType("application/json").body(item)
            .when().post().then().log().all().statusCode(200).extract().path("status");
  ...
  public void negativeStatisticalValues(){
    ...
    given().contentType("application/json").body(item).when().post().then().log().all().statusCode(200);
  }
  ...
  public void createWithStringSellerId(){
    ...
    given().contentType("application/json").body(body)
            .when().post().then().log().all().statusCode(400).body("status", equalTo("не передано тело объявления"));
